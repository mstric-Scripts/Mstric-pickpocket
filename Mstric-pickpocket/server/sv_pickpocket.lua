...
RegisterNetEvent('mstric_pickpocket:server:pickPocketNpc', function(netId)
    local src = source
    ...
    local zoneId = lib.callback.await('mstric_pickpocket:client:getZoneName', src, pos)
    ...
    if math.random() > Server.Luck then
        ...
        TriggerClientEvent('mstric_pickpocket:client:failedPickpocket', src, netId)
    end
